...
# Objective
m.setObjective(quicksum(ECost[c][p]*X[c,p] for c in C for p in P), GRB.MINIMIZE)
...
for c in C:
 ...
 # Minimum 80% virus eradication probability
m.addConstr(quicksum(X[c,p]*LogProbs[p] for c in C for p in P) >= np.log(0.8))
